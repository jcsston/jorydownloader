   ...
   for(i=0;i<hdrl_len;)
   {
      ...
      else if(strncasecmp((char *)hdrl_data+i,"strf",4)==0)
      {
         ...
         if(lasttag == 1)
         {
            ...
            memcpy(&bih, hdrl_data + i, sizeof(BITMAPINFOHEADER_avi));
            AVI->bitmap_info_header = (BITMAPINFOHEADER_avi *)malloc(bih.bi_size);
	    ...
	    memcpy(AVI->compressor2, hdrl_data+i+16, 4);
         ...
         {
            WAVEFORMATEX_avi *wfe;
	    char *nwfe;
            int wfes;
            
            if ((hdrl_len - i) < sizeof(WAVEFORMATEX_avi))
              wfes = hdrl_len - i;
            else
              wfes = sizeof(WAVEFORMATEX_avi);
            wfe = (WAVEFORMATEX_avi *)malloc(sizeof(WAVEFORMATEX_avi));
            if (wfe != NULL) {
              memset(wfe, 0, sizeof(WAVEFORMATEX_avi));
	      memcpy(wfe, hdrl_data + i, wfes);
	      if (wfe->cb_size != 0) {
		nwfe = (char *)realloc(wfe, sizeof(WAVEFORMATEX_avi) +
				       wfe->cb_size);
		if (nwfe != 0) {
		  off_t lpos = lseek(AVI->fdes, 0, SEEK_CUR);
		  lseek(AVI->fdes, header_offset + i + sizeof(WAVEFORMATEX_avi),
			SEEK_SET);
		  wfe = (WAVEFORMATEX_avi *)nwfe;
		  nwfe = &nwfe[sizeof(WAVEFORMATEX_avi)];
