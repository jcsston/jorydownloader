...
void ZipArchiveWriter::AddFile(const std::string &sourceFilename) {
	...
	if (inputFile != NULL) {
		...
		wxUint32 fileLength = ftell(inputFile);
...
void ZipArchiveWriter::AddFile(const std::string &newFile, const void *data, unsigned long size) {
#ifdef _DEBUG
	...
	memcpy(&fileHeader.last_mod_file_date, &ddDate, sizeof(wxUint16));
	memcpy(&fileHeader.last_mod_file_time, &dtTime, sizeof(wxUint16));
  ...
  if (ret == Z_OK) {
    ...
    fileHeader.uncompressed_size = size;
		...
		fileHeader.crc32 = crc32(fileHeader.crc32, (const unsigned char *)data, size);
